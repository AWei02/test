    ...
    async def _call_api(
        ...
        **generate_kwargs: Any,
        ...
        try:
            ...
            # Keep credentials and message contents out of logs.  The request
            # shape is enough to diagnose provider-side 4xx responses.
            logger.exception(
                "DeepSeek completion failed: model=%s stream=%s "
                "message_roles=%s tools=%s extra_body_keys=%s",
                model_name,
                self.stream,
                [message.role for message in messages],
                [tool.get("function", {}).get("name") for tool in fmt_tools],
                sorted(kwargs.get("extra_body", {})),
            )
            raise
