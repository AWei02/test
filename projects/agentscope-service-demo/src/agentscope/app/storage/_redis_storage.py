...
class RedisStorage(StorageBase):
    ...
    async def delete_schedule(
        self, user_id: str, schedule_id: str, *, preserve_sessions: bool = False,
    ) -> bool:
        """Delete a cron task record and optionally retain its sessions."""
        key = self._key(
            self.key_config.schedule,
            user_id=user_id,
            schedule_id=schedule_id,
        ...
        if not preserve_sessions:
            # Cascade only for administrative/agent lifecycle deletion.
    ...
    async def list_knowledge_documents_with_expired_lease(
        ...
        """
        now = now or datetime.now()
        terminal = {"ready", "error", "awaiting_parse_confirmation", "awaiting_chunk_confirmation"}
