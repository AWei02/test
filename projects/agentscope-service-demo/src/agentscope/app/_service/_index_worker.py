...
class IndexWorker:
    ...
    def __init__(
        ...
        resource_access_service: Any = None,
        document_stage_processor: Any = None,
        **kwargs: Any,
    ) -> None:
        ...
        self._document_stage_processor = document_stage_processor
    ...
    async def _run_pipeline(
        ...
        if record.data.manual_stages:
            if record.status not in ('pending', 'parsing', 'chunking', 'indexing'):
                return  # Duplicate delivery must not cross a human confirmation gate.
            if not self._document_stage_processor:
                raise ValueError('索引进程未配置人工确认流程')
            await self._document_stage_processor(record, self)
            return
        ...
        if self._document_parser:
            media_type = mimetypes.guess_type(data.filename)[0] or media_type
        if not media_type:
            raise ValueError(
        ...
        if self._document_parser:
            sections = await self._document_parser(record, file_bytes, self._storage,
                self._blob_store, self._resource_access_service)
        elif data.parsing_config:
            raise ValueError('当前索引工作进程未配置文档解析适配器')
        else:
