...
class ChatService:
    ...
    async def run(
        ...
        """
        try:
            observer = getattr(self, "audit_observer", None)
            if observer is None:
                await self._run_impl(user_id, session_id, agent_id, input_msg)
            else:
                async with observer.run(user_id, session_id, agent_id, input_msg):
                    await self._run_impl(user_id, session_id, agent_id, input_msg)
    ...
    async def _close_failed_reply(
        ...
        if observer is not None:
            observer.error(error, reply_msg.id)
                ...
                if (
                    ...
                    for kb_id in kb_cfg.knowledge_base_ids:
                        try:
                            ...
                            from ._document_access import AuthorizedKnowledge
                            knowledge = AuthorizedKnowledge(knowledge, self._access, self._storage,
                                                            user_id, kb_record.user_id, kb_id)
                        except Exception:  # pylint: disable=broad-except
