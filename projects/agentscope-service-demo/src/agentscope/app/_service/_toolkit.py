...
"""
    ...
    # Optional application workspace policy, applied before tools are exposed.
    filter_access = getattr(workspace, "filter_tool_access", None)
    if filter_access is not None:
        tools, tool_groups = filter_access(tools, tool_groups)
