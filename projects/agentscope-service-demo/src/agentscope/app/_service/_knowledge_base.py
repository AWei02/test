...
from ._document_access import AuthorizedKnowledge, can_read_document
...
class KnowledgeBaseService:
    ...
    async def register_document(
        ...
        access_users: list[str] | None = None,
        ...
        prepare_parsing = getattr(self._access._policy, 'prepare_document_parsing', None)
        if prepare_parsing:
            parsing_config = await prepare_parsing(self._access, user_id, knowledge_base_id, filename, parsing_config)
            if size > 50 * 1024 * 1024:
                raise HTTPException(413, '文件超过 50 MB 解析限制，请拆分后上传')
        prepare_acl = getattr(self._access._policy, 'prepare_document_acl', None)
        if prepare_acl:
            access_mode, access_users, access_roles = prepare_acl(knowledge_base_id, access_mode, access_users, access_roles)
        access_mode = access_mode or 'inherit'
        if access_mode not in ('inherit', 'private', 'custom') or (access_mode == 'custom' and not (access_users or access_roles)):
            raise HTTPException(400, '请选择有效的文件访问范围')
        ...
        record = KnowledgeDocumentRecord(
            ...
            data=KnowledgeDocumentData(
                ...
                access_users=access_users or [],
                access_roles=access_roles or [],
                parsing_config=parsing_config,
                manual_stages=bool(prepare_parsing),
            ),
        )
        try:
    ...
    async def list_documents(
        ...
        *,
        ...
        records = [r for r in records if can_read_document(self._access, user_id, r)]
    ...
    async def get_document_status(
        ...
        for document_id in document_ids:
            ...
            if record_doc is not None and can_read_document(self._access, user_id, record_doc):
    ...
    async def get_document(
        ...
        if document is None or not can_read_document(self._access, user_id, document):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def delete_document(
        ...
        for artifact in record.data.parsing_result.get('artifacts', {}).values():
            await self._delete_blob_quietly(artifact['uri'])
        if record.data.chunk_draft.get('uri'):
            await self._delete_blob_quietly(record.data.chunk_draft['uri'])
    ...
    async def _resolve_knowledge(
        ...
        try:
            ...
            return AuthorizedKnowledge(knowledge, self._access, self._storage,
                                       user_id, record.user_id, knowledge_base_id)
