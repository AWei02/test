...
class ChannelGateway:
    ...
    async def _ensure_session(
        ...
        """
        configure = getattr(self._workspace_manager, 'configure_channel_session', None)
        overrides = await configure(record, agent_id, session_id) if configure else {}
        ...
        if existing is not None:
            if overrides:
                await self._storage.upsert_session(record.user_id, agent_id, existing.config.model_copy(update=overrides), session_id=session_id)
            return
