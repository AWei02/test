...
class SchedulerManager:
        ...
        async def _trigger() -> None:
            ...
            try:
                if record.data.stateful:
                    stateful_session_id = (
                        f"{record.id}_stateful"
                        if record.data.workspace_version == 0
                        else f"{record.id}_stateful_{record.data.workspace_version}"
                    )
                    logger.info(
                ...
                # Bind only new/future schedule sessions. The helper never
                # overwrites an existing binding, preserving old history.
                if record.data.project_id:
                    try:
                        from project_files import bind_schedule_session

                        bind_schedule_session(
                            record.user_id,
                            record.agent_id,
                            session.id,
                        ...
                        logger.warning("Project workspace helper is unavailable")
    ...
    async def run_now(self, record: ScheduleRecord) -> None:
        """Execute one schedule run immediately without changing its cron job.

        This intentionally reuses the exact trigger path used by APScheduler,
        so session ownership, stateful behaviour and inbox delivery remain
        identical to a normally scheduled execution.
        """
        if not record.data.enabled:
            raise ValueError("Cannot run a disabled schedule")
        await self._build_trigger(record)()
