...
async def delete_knowledge_base(
    ...
    Args:
        ...
        service (`KnowledgeBaseService`):
            Injected knowledge base service.
    """
    await service._require_edit(user_id, knowledge_base_id)
    policy = getattr(request.app.state, 'resource_access_policy', None)
    references = getattr(policy, 'knowledge_references', None)
    grant_ids = references(knowledge_base_id) if references else []
    if grant_ids:
        raise HTTPException(409, '知识库仍被访问授权引用，不能删除。请先解除授权。授权编码：' + '、'.join(grant_ids))
...
async def upload_knowledge_document(
    ...
    access_mode: str | None = Form(default=None, pattern="^(inherit|private|custom)$"),
    access_users: str | None = Form(default=None),
    ...
    """
    try:
        parsing = json.loads(parsing_config) if parsing_config else None
        if parsing is not None and not isinstance(parsing, dict):
            raise ValueError()
    except (ValueError, TypeError):
        raise HTTPException(400, '解析配置必须是有效的 JSON 对象') from None
    record = await service.register_document(
        ...
        access_users=[v.strip() for v in (access_users or '').split(',') if v.strip()],
        access_roles=[v.strip() for v in (access_roles or '').split(',') if v.strip()],
