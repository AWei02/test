...
async def delete_mcp(
    ...
    surprise the user.
    """
    if await storage.get_mcp(user_id, mcp_id) is None:
        raise HTTPException(404, 'MCP 不存在')
    policy = getattr(request.app.state, 'resource_access_policy', None)
    references = getattr(policy, 'mcp_references', None)
    grant_ids = references(mcp_id) if references else []
    if grant_ids:
        raise HTTPException(409, 'MCP仍被访问授权引用，不能删除。请先解除授权。授权编码：' + '、'.join(grant_ids))
