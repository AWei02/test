...
import type { UpdateScheduleRequest } from "@/api";
...
export function ScheduleDetailDrawer({
  ...
  const exportSchedule = () => {
    const payload = {
      ...
      schedule: {
        ...
        chat_model_config: schedule!.data.chat_model_config,
        ...
        ended_at: schedule!.data.ended_at,
      },
    };
    ...
    a.href = url;
    a.download = `${data.name.replace(/[^\w.-]+/g, "_") || "schedule"}.json`;
    a.click();
  ...
  const agentName =
    agents.find((a) => a.id === schedule.agent_id)?.data.name ??
    schedule.agent_id;
                ...
                {!sessionsLoading &&
                  sessions.map((session) => (
                    <div
                      ...
                      onClick={() => {
                        navigate(`/chat/${schedule.agent_id}/${session.id}`);
                        onOpenChange(false);
                      }}
                    >
