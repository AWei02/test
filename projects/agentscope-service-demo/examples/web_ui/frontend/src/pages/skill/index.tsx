...
import { SkillPackageUpload } from '@/components/SkillPackageUpload';
		...
		<div className="flex size-full p-2 gap-2">
			...
			<main className="flex-1 min-w-0 min-h-0 overflow-hidden rounded-[22px] bg-card shadow-panel">
				{hubId ? (
					...
					<MinePanel skills={skills} loading={skillsLoading} onRemove={(id) => {if(window.confirm('确定删除这个技能吗？确认后将检查授权占用，有引用时无法删除。')) void remove(id);}} />
				)}
			</main>
		</div>
	);
