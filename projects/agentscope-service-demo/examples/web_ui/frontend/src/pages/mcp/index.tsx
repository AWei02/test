...
function MinePanel({ mcps, loading, onEdit, onRemove, onSaved }: MinePanelProps) {
	const { t } = useTranslation();
	const [query, setQuery] = useState('');
					...
					{shown.map((mcp) => (
						<Item key={mcp.id}>
							...
							<ItemActions className="gap-1">
								{!mcp.hub_id && (
									<CustomMCPDialog editId={mcp.id} onSaved={onSaved} />
								)}
								{mcp.version && (
			...
			<main className="flex-1 min-w-0 min-h-0 overflow-hidden rounded-[22px] bg-card shadow-panel">
				{hubId ? (
					...
					<MinePanel
						...
						loading={mcpsLoading}
						onEdit={setEditing}
						onRemove={(id) => {if(window.confirm('确定删除这个 MCP 吗？确认后将检查授权占用，有引用时无法删除。')) void remove(id);}}
						onSaved={refetchMcps}
					/>
				)}
			</main>
