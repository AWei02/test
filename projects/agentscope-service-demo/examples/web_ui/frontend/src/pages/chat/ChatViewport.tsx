...
import { SkillFiles } from '@/components/chat/SkillFiles';
...
interface ChatViewportProps {
	...
	onStartSession?: (model: ChatModelConfig) => Promise<void>;
}
...
export function ChatViewport({
	...
	const {
		...
		removeSkill,
	} = useWorkspace(isAdmin ? agentId : null, isAdmin ? sessionId : null);
	...
	const selectedModelCard = useMemo(() => {
		...
		return null;
	}, [groups, selectedModel?.type, selectedModel?.model, selectedModel?.credential_id]);
	...
	const getFirstAvailableModel = (): ChatModelConfig | null => {
		// A credential may expose only embeddings/rerank models, or its
		// catalog may be unavailable. Keep looking for a usable chat model.
		for (const [type, items] of Object.entries(groups)) {
			for (const { credential, models } of items) {
				for (const model of models as { name?: string; id?: string }[]) {
	...
	const handleLlmChange = async (config: ChatModelConfig | null) => {
		if (!config) return;
		if (!sessionId) {
			if (!agentId || !onStartSession || configPending) return;
			setConfigPending(true);
			try {
				await onStartSession(config);
			} catch {
				// The API client reports creation errors; don't display an unsaved model.
			} finally {
				setConfigPending(false);
			}
			return;
				...
				<ResizablePanelGroup orientation="horizontal">
					...
					>
						<div className="flex flex-col flex-1 min-h-0 min-w-0 overflow-x-hidden p-2">
							<div className="mx-4 my-2 flex min-w-0 items-center gap-2">
								{agentId && sessionId && (
									<SkillFiles
										key={`skill-files/${agentId}/${sessionId}`}
										sessionName={view?.session.config.name}
										actions={
											!isAdmin ? (
												<CapabilityPicker
													key={`capabilities/${agentId}/${sessionId}`}
													agentId={agentId}
													sessionId={sessionId}
												/>
											) : null
							...
							<div className="flex flex-row gap-x-2 justify-between">
								...
								<div className="flex shrink-0 flex-row gap-x-1">
									<LlmSelect
										...
										onAddCredential={
											isAdmin ? () => setCredentialOpen(true) : undefined
										}
									...
									<ModelParametersPopover
										...
										disabled={configPending || !sessionId}
									/>
									{isAdmin && (
							...
							<div className="flex flex-1 justify-center min-h-0 overflow-hidden relative [--chat-content-w:48rem]">
								<ChatContent
									...
									fileProcessor={async (file) => {
										if (!agentId || !sessionId) throw new Error('请先创建会话');
