...
export interface ChatRequest {
  ...
  session_id: string;
  input:
    Msg | Msg[] | UserConfirmResultEvent | ExternalExecutionResultEvent | null;
}
...
export interface ModelCard {
  ...
  /** Explicit capability returned by a provider's live model catalogue. */
  category?: "llm" | "rerank" | "embedding" | "stt" | "tts";
}
...
export interface KnowledgeBaseStatusCounts {
  awaiting_parse_confirmation?: number;
  awaiting_chunk_confirmation?: number;
  pending: number;
  parsing: number;
  chunking: number;
  indexing: number;
...
export type KnowledgeDocumentStatus =
  "pending" | "parsing" | "chunking" | "indexing" | "ready" | "error" | "awaiting_parse_confirmation" | "awaiting_chunk_confirmation";
...
export type ChannelState =
  "stopped" | "connecting" | "retrying" | "connected" | "failed";
