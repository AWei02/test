import { ApiError, client, serviceUrl, getAuthHeaders } from './client';
import type {
...
export interface UploadDocumentOptions {
	permissions?: {
		access_mode: 'inherit' | 'private' | 'custom';
		access_users: string[];
		access_roles: string[];
	};
...
function uploadDocumentXhr(
	...
	if (options.permissions) {
		formData.append('access_mode', options.permissions.access_mode);
		formData.append('access_users', options.permissions.access_users.join(','));
		formData.append('access_roles', options.permissions.access_roles.join(','));
	}
	if (options.parsing) formData.append('parsing_config', JSON.stringify(options.parsing));
	...
	return new Promise((resolve, reject) => {
		...
		const url = serviceUrl(`/knowledge_bases/${knowledgeBaseId}/documents`);
...
export const knowledgeBaseApi = {
	...
	documentContentUrl: (
		...
		const url = serviceUrl(`/knowledge_bases/${knowledgeBaseId}/documents/${documentId}`);
