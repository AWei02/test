...
export const getAuthHeaders = (): Record<string, string> => ({
	Authorization: `Bearer ${localStorage.getItem('login_key') ?? ''}`,
});
export const getUserId = () => localStorage.getItem('username') ?? '';
export const serviceUrl = (path: string) => new URL('/api/service' + path, window.location.origin);
...
async function streamRequest(path: string, options: RequestOptions = {}): Promise<Response> {
	...
	const url = baseUrl ? new URL(path, baseUrl) : serviceUrl(path);
	if (params) {
