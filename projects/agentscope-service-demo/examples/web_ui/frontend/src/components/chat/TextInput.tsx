...
interface TextInputProps {
	 ...
	 */
	fileProcessor: (file: File) => Promise<ContentBlock | ContentBlock[] | null>;
	/**
...
export const TextInput = forwardRef<TextInputRef, TextInputProps>(
	(
		...
		const handleSend = () => {
			...
			files.forEach((f) => {
				if (f.status === 'done' && f.block) {
					blocks.push(f.block, ...(f.additionalBlocks ?? []));
				}
			});
		...
		const addFiles = (selected: File[]) => {
			if (attachDisabled) {
				setAttachmentError('当前无法添加附件，请先选择支持附件的模型。');
				return;
			}
			setAttachmentError('');
			...
			selected.forEach((original) => {
				...
				const accepted =
					...
					allowedInputTypes
						...
						.some((value) => {
							...
							if (type.startsWith('.'))
								return original.name.toLowerCase().endsWith(type);
							if (type === '*/*') return true;
							return type.endsWith('/*')
								? mediaType.startsWith(type.slice(0, -1))
								: mediaType === type;
						});
				if (!accepted) {
				...
				setFiles((prev) => [
					...prev,
					{ id, name: file.name, status: 'processing', block: null },
				]);
				Promise.resolve()
					.then(() => fileProcessor(file))
					.then((result) => {
						const [block, ...additionalBlocks] = Array.isArray(result)
							? result
							: result
								? [result]
								: [];
						setFiles((prev) =>
							block
								? prev.map((entry) =>
										entry.id === id
											? { ...entry, status: 'done', block, additionalBlocks }
											: entry,
									)
					...
					.catch((error: unknown) => {
						...
						setAttachmentError(
							`附件处理失败：${file.name}。${error instanceof Error ? error.message : '请重试'}`,
						);
					});
			});
			if (rejected.length)
				setAttachmentError(`当前模型不支持这些文件类型：${rejected.join('、')}`);
		};
		...
		return (
			...
			>
				{isDragging && (
					<div
						role="status"
						className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center rounded-[28px] border-2 border-dashed border-primary bg-background/95 p-4 text-sm text-center"
					>
						{attachDisabled ? '当前无法添加附件' : '松开鼠标添加附件（支持多个文件）'}
					</div>
				)}
				{attachmentError && (
