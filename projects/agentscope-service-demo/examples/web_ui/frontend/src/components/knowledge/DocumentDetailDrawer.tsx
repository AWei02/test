...
import { permissionSummary } from './DocumentPermissions';
import { ParsingDetail } from './DocumentParsing';
...
/**
 * Main-area document detail, replacing the list until the user goes back.
 */
export function DocumentDetailView({ onBack, knowledgeBaseId, document, editable = false }: Props) {
	const { t } = useTranslation();

	return (
			<section aria-label="文档详情" className="flex h-full min-h-0 w-full min-w-0 flex-col gap-y-4 overflow-hidden">
                <Button variant="ghost" className="w-fit shrink-0" onClick={onBack}>
                    <ChevronLeft className="size-4" />返回文档列表
                </Button>
				<header className="shrink-0 space-y-2 lg:group-has-[[data-testid=chunking-preview]]/knowledge-tabs:max-w-[calc(50%-8px)]">
					<h2 className="flex min-w-0 items-start gap-x-2 font-medium">
				...
				<Tabs defaultValue={document.status === 'ready' ? 'chunks' : 'parsing'} className="flex min-h-0 flex-1 flex-col gap-y-3">
					<TabsList className="w-fit shrink-0">
                        ...
                        <TabsTrigger value="parsing">处理 / 审核</TabsTrigger>
					</TabsList>
                    <TabsContent value="parsing" className="min-h-0 flex-1 overflow-hidden"><ParsingDetail kbId={knowledgeBaseId} docId={document.id} editable={editable} /></TabsContent>
                    <TabsContent value="permissions" className="min-h-0 flex-1 overflow-y-auto space-y-4 pr-2 text-sm">
                        <p>{permissionSummary(document)}</p>
                        {!!document.access_users?.length && <p>可读用户：{document.access_users.join('、')}</p>}
                        {!!document.access_roles?.length && <p>可读角色：{document.access_roles.join('、')}</p>}
                        <p className="text-xs text-muted-foreground">文件权限仅对已有知识库访问权的用户生效。所有者与管理员始终可读。</p>
                        <p className="text-xs text-muted-foreground">文件上传：{new Date(document.created_at).toLocaleString()}</p>
                        <p className="text-xs text-muted-foreground">最后更新：{new Date(document.updated_at).toLocaleString()}</p>
                    </TabsContent>
					...
					<TabsContent value="preview" className={mediaType(document) === 'application/pdf' ? 'min-h-0 flex-1 overflow-hidden' : 'min-h-0 flex-1 overflow-y-auto'}>
						<PreviewTab
							open={true}
							knowledgeBaseId={knowledgeBaseId}
							document={document}
...
function PreviewTab({ open, knowledgeBaseId, document }: TabProps) {
	...
	if (isPdf && tokenUrl) {
		return <iframe src={tokenUrl} title={document.filename} className="block h-full min-h-0 w-full border-0" />;
	}
	if (isImage && tokenUrl) {
