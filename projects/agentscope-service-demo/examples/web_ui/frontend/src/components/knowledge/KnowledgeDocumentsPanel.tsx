import { AlertCircle, CheckCircle2, FileText, Loader2, Plus, Trash2, X, Pencil } from 'lucide-react';
import { client } from '@/api/client';
import { PermissionDialog, permissionSummary } from './DocumentPermissions';
...
import { DocumentDetailView } from '@/components/knowledge/DocumentDetailDrawer.tsx';
...
import { useUploadContext } from '@/context/uploadContextState';
...
const TERMINAL_SERVER_STATUSES: KnowledgeDocumentStatus[] = ['ready', 'error', 'awaiting_parse_confirmation', 'awaiting_chunk_confirmation'];
...
function StatusBadge({ phase }: { phase: UploadPhase }) {
	...
	const label = phase === 'awaiting_parse_confirmation' ? '待确认解析' : phase === 'awaiting_chunk_confirmation' ? '待确认分片' : t(`knowledge.document.status.${phase}`);
	const tone = (() => {
		switch (phase) {
			case 'ready':
	...
	return (
		...
		>
			{phase === 'ready' || phase === 'awaiting_parse_confirmation' || phase === 'awaiting_chunk_confirmation' ? (
...
interface RowViewProps {
    editable: boolean;
    onEdit: (doc: KnowledgeDocumentView) => void;
	row: Row;
...
function RowView({ row, onCancel, onDismiss, onDelete, onOpen, onEdit, editable }: RowViewProps) {
	const { t } = useTranslation();
	...
	const showProgress = !['ready', 'cancelled', 'awaiting_parse_confirmation', 'awaiting_chunk_confirmation'].includes(phase);

	// Ready server rows open the detail drawer (chunks + preview).
	const openable = row.kind === 'server' && TERMINAL_SERVER_STATUSES.includes(row.doc.status);
			...
			<div className="flex items-start gap-x-3">
				...
				<div className="flex min-w-0 flex-1 flex-col gap-y-0.5">
					<div className="flex items-center gap-x-2">
                        ...
                        {row.kind === 'server' && <span className="rounded-md bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">{permissionSummary(row.doc)}</span>}
					</div>
				...
				<div
					className={editable ? "flex shrink-0 items-center gap-x-1" : "hidden"}
					onClick={(e) => e.stopPropagation()}
				>
                    {editable && row.kind === 'server' && <Button variant="ghost" size="icon-xs" title="编辑文件权限" aria-label="编辑文件权限"
                        disabled={!TERMINAL_SERVER_STATUSES.includes(row.doc.status)} onClick={() => onEdit(row.doc)}><Pencil className="size-3.5" /></Button>}
...
export function KnowledgeDocumentsPanel({ knowledgeBaseId, editable = false }: KnowledgeDocumentsPanelProps) {
	const { t } = useTranslation();
    ...
    const [editTarget, setEditTarget] = useState<KnowledgeDocumentView | null>(null);
    const [pendingFiles, setPendingFiles] = useState<File[]>([]);
    const [defaultMode, setDefaultMode] = useState<'inherit' | 'private'>('inherit');
    const [permissionFilter, setPermissionFilter] = useState('all');
    useEffect(() => { if (editable) client.get<{access_mode: 'inherit' | 'private'}>(`/portal/knowledge-permissions/${knowledgeBaseId}`)
        .then(r => setDefaultMode(r.access_mode)).catch(() => {}); }, [knowledgeBaseId, editable]);
	...
	const handleFiles = useCallback(
		(files: FileList | File[]) => {
			...
			if (editable) setPendingFiles(accepted);
		},
		[editable, extensionSet, t],
	);
    ...
    // Keep this panel mounted so filters and upload tracking survive going back.
    if (detailTarget) {
        const currentRow = rows.find(row => row.kind === 'server' && row.doc.id === detailTarget.id);
        return <DocumentDetailView
            key={detailTarget.id}
            onBack={() => { setDetailTarget(null); void refetch(); }}
            knowledgeBaseId={knowledgeBaseId}
            document={currentRow?.kind === 'server' ? currentRow.doc : detailTarget}
            editable={editable}
        />;
    }
		...
		<div className="flex flex-col gap-y-4">
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-x-2">
                    ...
                    <select aria-label="筛选文件权限" className="rounded-lg border bg-background px-2 py-1 text-xs" value={permissionFilter} onChange={e => setPermissionFilter(e.target.value)}>
                        <option value="all">全部权限</option><option value="inherit">继承权限</option><option value="private">仅所有者</option><option value="custom">自定义</option>
                    </select>
					{editable && <Button variant="outline" size="sm" onClick={onUploadClick}>
			...
			>
				{rows.length === 0 && loading ? (
					...
					<Empty className="border-none py-6">
						...
						<EmptyContent>
							{editable && <Button size="sm" onClick={onUploadClick}>
								<Plus className="size-3.5" />
					...
					<div className="flex flex-col gap-y-2 p-2">
						{rows.filter(row => permissionFilter === 'all' || (row.kind === 'server' && (row.doc.access_mode ?? 'inherit') === permissionFilter)).map((row) => (
							<RowView
            ...
            {(editTarget || pendingFiles.length > 0) && <PermissionDialog document={editTarget ?? undefined} files={pendingFiles.length ? pendingFiles : undefined}
                kbId={knowledgeBaseId} defaultMode={defaultMode} onClose={() => { setEditTarget(null); setPendingFiles([]); }} onSaved={() => { void refetch(); }}
                onUpload={(permissions, parsing) => enqueue(knowledgeBaseId, pendingFiles, permissions, parsing)} />}
