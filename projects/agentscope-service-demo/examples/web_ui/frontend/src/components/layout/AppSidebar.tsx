...
import { BrandLogo } from "@/components/BrandingSettings";
...
export function AppSidebar() {
  const { isAdmin, data: user } = usePortal();
  ...
  if (!isAdmin)
    return (
      <aside className="w-14 flex flex-col items-center py-4 border-r bg-white gap-5">
        <button title="聊天" onClick={() => navigate("/chat?ordinary=1")}>
          <BotMessageSquare className="size-5" />
        </button>
        <button title="日程" onClick={() => navigate("/schedule")}>
          <Calendars className="size-5" />
        </button>
        <button
          className="mt-auto"
          title={`当前用户：${user?.username}，切换用户`}
    ...
    >
      <SidebarHeader>
        <button
          ...
          aria-label="管理后台"
          className="flex items-center justify-center size-8 mt-2 rounded-full bg-transparent focus-visible:ring-2"
        >
          <BrandLogo />
        </button>
      </SidebarHeader>
